...
EXPLORE = 3000000
BATCH_SIZE = 20 # every how many eposides do a parameter update
GAMMA = 0.99
r_reward_moving_average = 0
average_step_record = list()
BASE_PATH = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
file_path = BASE_PATH + '/pg_step_record.txt'
...
def stats_summary(rollouts, records, verbose=True):
    ...
    for rollout in rollouts:
        total_step += rollout.steps
    average_step = total_step/len(rollouts)
    average_step_record.append(average_step)
    print("average step record %s "%(average_step))
    ...
    if (played_steps > BATCH_SIZE * 19):
        ...
        if t % 1000 == 0 :
            ...
            with open(file_path,'a') as f:
                for step in average_step_record:
                    f.write(str(step) + " ")
                average_step_record = []
