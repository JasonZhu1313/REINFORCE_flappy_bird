...
file_path = BASE_PATH + "/process_record.txt"
...
def trainNetwork(model, args):
    ...
    if args['mode'] == 'Run':
        ...
        # load_mode
        model = Net.load('./model/model200.bigdl','./model/model200.bin')
    ...
    while (True):
        ...
        if terminal:
            step_record.append(livinging_step)
            livinging_step = 0
        else:
            livinging_step += 1
        x_t1 = x_t1 / 255.0
        ...
        if t > OBSERVE:
            ...
            model.fit(state_t,targets,BATCH)
        ...
        if t % 500 == 0 and args['mode'] == 'Train':
            with open(file_path,"a") as f:
                for step in step_record:
                    f.write(str(step) + " ")
            step_record = []
            print("Now we save model")
            print("./model/model"+str(t)+".bigdl")
            print("./model/model"+str(t)+".bin")
        ...
        if t <= OBSERVE:
            ...
            state = "train"
        print("Living_steps ",livinging_step)
